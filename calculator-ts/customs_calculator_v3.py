...
def calc_excise(spec: VehicleSpec) -> float:
    ...
    if spec.engine_type == EngineType.ELECTRIC:
        # Для электромобилей — ставка по диапазону на всю мощность
...
def calc_customs_fee(cost_rub: float, importer_type: ImporterType) -> float:
    """Расчет таможенного сбора по таблице ПП РФ № 1637 для всех категорий импортёров"""
...
def calc_car_duty(spec: VehicleSpec, cost_eur: float) -> float:
    ...
    if spec.importer_type in [ImporterType.PHYS_PERSONAL, ImporterType.PHYS_RESALE]:
        ...
        # Определяем тип топлива из spec.fuel_type
        engine_key = spec.fuel_type.value if isinstance(spec.fuel_type, FuelType) else str(spec.fuel_type)

        # ЗАГОТОВКА: если заполнены таблицы ЕТТ, используем их (по диапазонам объема)
        ett_tables = RATES_2025.get('CAR_DUTY_ETT', {}).get(engine_key, {})
        ett_table = ett_tables.get(age_key, [])
        if ett_table:
            rate_info = find_rate_by_value(spec.engine_volume_cc, ett_table, 'max_cc')
            rate = rate_info.get('rate', RATES_2025['CAR_DUTY']['commercial'][engine_key][age_key])
            min_rate_eur_cc = rate_info.get('min_eur_cc', RATES_2025['CAR_DUTY']['commercial']['min_rates'][engine_key][age_key])
        else:
