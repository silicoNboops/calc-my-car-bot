...
## V4 non-car: таможенные сборы

В версионированной фикстуре V4 non‑car (`customs_rates_v4_noncar_2025_08_17.json`) заполнен блок `customs_fees` 
с официальными порогами (ПП РФ №1637), идентичными таблице `CUSTOMS_FEES` в эталонном TS‑скрипте. 
Это обеспечивает полное совпадение суммы таможенного сбора между API и TS.
...
### Примечание: игнорирование шаблонных файлов

По умолчанию команда игнорирует JSON‑файлы, в имени которых встречается `template` (например, `customs_rates_template.json`). Это предотвращает случайную загрузку примеров в dev/tests.

Если нужно явно загрузить шаблонный файл, используйте один из вариантов:

- Переименуйте файл, добавив версионный тег в имя, и вызовите с `--version-tag <тот_же_тег>`.
- Или поместите содержимое в новый файл с нужным именем (без `template`) в директорию фикстур.
