...
### Примечания по логике калькулятора (v5, без фиче-флагов)

- Текущая референсная версия логики — v5 (актуальные правила без фиче-флагов).
...
- Акциз начисляется только для легковых авто (`vehicle_type == "car"`). Для `quad`/`snowmobile`/`motorcycle` акциз = 0.
  Для EV акциз рассчитывается по прогрессивной шкале согласно `AcciseRate` вне зависимости от типа пользователя (ФЛ/ЮЛ)
  и возраста авто.
- Пошлина (duty) для EV — 15% от цены (евро), без минималки «1 EUR/см³».
- Утильсбор соответствует принятой схеме: база берётся из `Settings.util_base`, для не-легковых применяется множитель
  8.625 и коэффициенты согласно текущим правилам.
...
- Новые единицы ставок пошлины в `DutyRate.unit`:
    - `EUR_HP` — фикс €/л.с. (используется для ФЛ старше 3 лет по quad/snowmobile и для ряда кейсов у ЮЛ по возрастным
      группам).
    - `PERCENT_HP` — процент от цены с минимальным €/л.с. (используется для ФЛ младше 3 лет по quad/snowmobile и для ЮЛ
